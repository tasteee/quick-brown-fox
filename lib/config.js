...
const { detectFramework } = require('./frameworks')
 ...
 * `source/` folder for the UI, and optionally a `server/` folder for a
...
function resolveConfig(cwd, cliOptions = {}) {
  ...
  const framework = detectFramework(
    pkg,
    cliOptions.framework || fileCfg.framework || (fileCfg.ui && fileCfg.ui.framework)
  )
  ...
  const filesystem = resolveFilesystem(fileCfg)
...
function resolveFilesystem(fileCfg) {
  const declared =
    fileCfg.filesystem !== undefined
      ? fileCfg.filesystem
      : fileCfg.permissions && fileCfg.permissions.filesystem

  if (!declared) return { enabled: false }
  if (declared === true) return { enabled: true }
  if (typeof declared === 'object') return Object.assign({ enabled: true }, declared)
  throw new Error('qbf.filesystem must be true or an object.')
}
