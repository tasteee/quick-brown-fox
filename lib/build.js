...
const { frameworkLabel } = require('./frameworks')
 ...
 *
 *   1. Bundle the renderer UI with Vite.
...
async function build(config, opts = {}) {
  ...
  log.step(`framework ${log.c.cyan(frameworkLabel(config.framework))}`)
  if (config.server) {
    ...
    JSON.stringify(
      {
        ...
        serverPort: config.server ? config.server.port : null,
